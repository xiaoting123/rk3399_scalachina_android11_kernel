...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    {
        //printk("Error occured in init_panel, no config sent");
        return 0;
    }
    
    //printk("Driver send config.");
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
    ...
    printk("Request input device for pen/stylus.");
    
    ts->pen_dev = input_allocate_device();
    if (ts->pen_dev == NULL)
    {
        //printk("Failed to allocate input device for pen/stylus.");
        return;
    }
    ...
    if (ret)
    {
        printk("Register %s input device failed", ts->pen_dev->name);
        return;
    }
}
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if ((finger == 0x00) && (CHIP_TYPE_GT9F == ts->chip_type))     // request arrived
    {
        ...
        if (ret < 0)
        {
           printk("Read request status error!");
           goto exit_work_func;
        } 
        ...
        case GTP_RQST_CONFIG:
            printk("Request for config.");
            ret = gtp_send_cfg(ts->client);
            if (ret < 0)
            {
                printk("Request for config unresponded!");
            }
            else
            {
                rqst_buf[2] = GTP_RQST_RESPONDED;
                gtp_i2c_write(ts->client, rqst_buf, 3);
                printk("Request for config responded!");
            }
            break;
            
        case GTP_RQST_BAK_REF:
            printk("Request for backup reference.");
            ...
            if (SUCCESS == ret)
            {
                ...
                printk("Request for backup reference responded!");
            }
            else
            {
                printk("Requeset for backup reference unresponed!");
            }
            break;
            
        case GTP_RQST_RESET:
            printk("Request for reset.");
            gtp_recovery_reset(ts->client);
            break;
            
        case GTP_RQST_MAIN_CLOCK:
            printk("Request for main clock.");
            ts->rqst_processing = 1;
            ret = gtp_main_clk_proc(ts);
            if (FAIL == ret)
            {
                printk("Request for main clock unresponded!");
            }
            else
            {
                printk("Request for main clock responded!");
                rqst_buf[2] = GTP_RQST_RESPONDED;
                gtp_i2c_write(ts->client, rqst_buf, 3);
                ts->rqst_processing = 0;
        ...
        default:
            printk("Undefined request: 0x%02X", rqst_buf[2]);
            rqst_buf[2] = GTP_RQST_RESPONDED;  
            gtp_i2c_write(ts->client, rqst_buf, 3);
            break;
        }
...
static s8 gtp_enter_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        if (ret < 0)
        {
            printk("failed to get backup-reference status");
        }
        ...
        if (status_buf[2] & 0x80)
        {
            ...
            if (FAIL == ret)
            {
                printk("failed to store bak_ref");
            }
        }
    }
#endif
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        for (retry = 0; retry < 10; ++retry)
        {
            ...
            if (FAIL == ret)
            {
                printk("failed to hold ss51 & dsp!");
                continue;
            }
            opr_buf[2] = 0x00;
            ...
            if (FAIL == ret)
            {
                printk("failed to get ss51 & dsp status!");
                continue;
            }
            if (0x0C != opr_buf[2])
            {
            ...
            if (FAIL == ret)
            {
                printk("failed to startup GT9XXF, process recovery");
        ...
        if (retry >= 10)
        {
            printk("failed to wakeup, processing esd recovery");
            gtp_esd_recovery(ts->client);
        }
        else
        {
            printk("GT9XXF gtp wakeup success");
        }
        return ret;
    }
#endif
    ...
    while(retry++ < 10)
    {
        ...
        {
            printk("Powerkey wakeup.");
        }
        else   
        {
            printk("Gesture wakeup.");
        }
 ...
 */
	...
	if (ret < 0) {
		printk("Failed to parse config data form device tree.");
		ts->pnl_init_error = 1;
		return -1;
	}
#else 
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        printk("Drv * Sen: %d * %d(key: %d), X_MAX: %d, Y_MAX: %d, TRIGGER: 0x%02x",
...
{
    s32 ret = 0;
printk("gtp_request_io_port  gtp_int_gpio :%d,gtp_rst_gpio:%dstart\n",gtp_int_gpio,gtp_rst_gpio);
    GTP_DEBUG_FUNC();
    ret = GTP_GPIO_REQUEST(gtp_int_gpio, "GTP INT IRQ");
    if (ret < 0) 
    {
	printk("gtp_request_io_port  gtp_int_gpio failed\n");
        printk("Failed to request GPIO:%d, ERRNO:%d", (s32)gtp_int_gpio, ret);
        ret = -ENODEV;
    }
    else
    {
    ...
    if (ret < 0) 
    {
	printk("gtp_request_io_port  gtp_rst_gpio failed\n");
        printk("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_rst_gpio,ret);
        ret = -ENODEV;
    }
...
s32 gtp_fw_startup(struct i2c_client *client)
{
    ...
    if(0xAA == opr_buf[0])
    {
        printk("IC works abnormally,startup failed.");
        return FAIL;
    }
    else
    {
        printk("IC works normally, Startup success.");
        opr_buf[0] = 0xAA;
...
static s32 gtp_esd_recovery(struct i2c_client *client)
{
    ...
    printk("GT9XXF esd recovery mode");
    for (retry = 0; retry < 5; retry++)
    {
        ret = gup_fw_download_proc(NULL, GTP_FL_ESD_RECOVERY); 
        if (FAIL == ret)
        {
            printk("esd recovery failed %d", retry+1);
            continue;
        }
        ret = gtp_fw_startup(ts->client);
        if (FAIL == ret)
        {
            printk("GT9XXF start up failed %d", retry+1);
            continue;
        }
        break;
    }
    ...
    if (retry >= 5)
    {
        printk("failed to esd recovery");
        return FAIL;
    }
    
    printk("Esd recovery successful");
    return SUCCESS;
}
...
static s32 gtp_bak_ref_proc(struct goodix_ts_data *ts, u8 mode)
{
    ...
    if (FAIL == ret)
    {
        ...
        {
            msleep(50);
            printk("/data not mounted.");
            return FAIL;
        }
        printk("check /data mount timeout...");
    }
    else
    {
        printk("/data mounted!!!(%d/%d)", ts->ref_chk_fs_times, GTP_CHK_FS_MNT_MAX);
    }
    ...
    if (NULL == p_bak_ref)
    {
        printk("Allocate memory for p_bak_ref failed!");
        return FAIL;
    }
    ...
    if (IS_ERR(ref_filp))
    { 
        printk("Failed to open/create %s.", GTP_BAK_REF_PATH);
    ...
    case GTP_BAK_REF_SEND:
        ...
        for (j = 0; j < ref_grps; ++j)
        {
            ...
            if (1 != ref_sum)
            {
                printk("wrong chksum for bak_ref, reset to 0x00 bak_ref");
            ...
            else
            {
                if (j == (ref_grps - 1))
                {
                    printk("backup-reference data in %s used", GTP_BAK_REF_PATH);
                }
            }
        }
        ...
        if (FAIL == ret)
        {
            printk("failed to send bak_ref because of iic comm error");
            goto bak_ref_exit;
        }
        break;
        
    case GTP_BAK_REF_STORE:
        printk("Store backup-reference");
        ret = i2c_read_bytes(ts->client, GTP_REG_BAK_REF, p_bak_ref, ts->bak_ref_len);
        if (ret < 0)
        {
            printk("failed to read bak_ref info, sending default back-reference");
            goto bak_ref_default;
        }
        ref_filp->f_op->llseek(ref_filp, 0, SEEK_SET);
    ...
    default:
        printk("invalid backup-reference request");
        break;
    }
    ret = SUCCESS;
    goto bak_ref_exit;
...
bak_ref_default:
    ...
    if (!IS_ERR(ref_filp))
    {
        printk("write backup-reference data into %s", GTP_BAK_REF_PATH);
        ref_filp->f_op->llseek(ref_filp, 0, SEEK_SET);
        ref_filp->f_op->write(ref_filp, (char*)p_bak_ref, ts->bak_ref_len, &ref_filp->f_pos);
    }
    if (ret == FAIL)
    {
        printk("failed to load the default backup reference");
    }
...
static s32 gtp_main_clk_proc(struct goodix_ts_data *ts)
{
    ...
    {
        printk("/data mounted!!!(%d/%d)", ts->clk_chk_fs_times, GTP_CHK_FS_MNT_MAX);
    }
    
    clk_filp = filp_open(GTP_MAIN_CLK_PATH, O_RDWR | O_CREAT, 0666);
    if (IS_ERR(clk_filp))
    {
        printk("%s is unavailable, calculate main clock", GTP_MAIN_CLK_PATH);
    }
    else
    {
        ...
        if (FAIL == ret)
        {
            ...
            printk("main clock data in %s is wrong, recalculate main clock", GTP_MAIN_CLK_PATH);
        }
        else
        { 
            printk("main clock data in %s used, main clock freq: %d", GTP_MAIN_CLK_PATH, p_main_clk[0]);
    ...
    printk("calibrate main clock: %d", ret);
    if (ret < 50 || ret > 120)
    {
        printk("wrong main clock: %d", ret);
        goto exit_main_clk;
    }
...
update_main_clk:
    ...
    if (FAIL == ret)
    {
        printk("update main clock failed!");
        return FAIL;
    }
    return SUCCESS;
...
void gtp_get_chip_type(struct goodix_ts_data *ts)
{
    ...
    if (FAIL == ret)
    {
        printk("Failed to get chip-type, set chip type default: GOODIX_GT9");
    ...
    printk("Chip Type: %s", (ts->chip_type == CHIP_TYPE_GT9) ? "GOODIX_GT9" : "GOODIX_GT9F");
}
...
printk("1111gtp_int_gpio:%d ,gtp_rst_gpio:%d\n",gtp_int_gpio,gtp_rst_gpio);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<======\n");
    ...
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION1<======\n");
...
//	}
...
#endif
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<2======\n");
    ...
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<3======\n");
    ret = gtp_request_io_port(ts);
    if (ret < 0)
   
     {
    printk("11hahahhahhahahahha====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<4======\n");
        printk("GTP request IO port failed.");
        kfree(ts);
        return ret;
    }
    
    printk("11====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<4======\n");
#if GTP_COMPATIBLE_MODE
...
#endif
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<4======\n");

    ret = gtp_i2c_test(client);
    if (ret < 0)
    {
        printk("I2C communication ERROR!ret:%d",ret);
    }
    ...
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<5======\n");
    ...
    {
        printk("create proc entry %s success", GT91XX_CONFIG_PROC_FILE);
    }
    ...
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<6======\n");
    ...
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<7======\n");
    ...
    init_wr_node(client);
#endif
    printk("====>HERE WE IN THE GOODIX_TS_PROBE FUNCTION<8======\n");
    return 0;
...
s32 gtp_i2c_read_no_rst(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    if ((retries >= 5))
    {    
        //printk("I2C Read: 0x%04X, %d bytes failed, errcode: %d!", (((u16)(buf[0] << 8)) | buf[1]), len-2, ret);
    }
    return ret;
}
...
s32 gtp_i2c_write_no_rst(struct i2c_client *client,u8 *buf,s32 len)
{
    ...
    if((retries >= 5))
    {
        //printk("I2C Write: 0x%04X, %d bytes failed, errcode: %d!", (((u16)(buf[0] << 8)) | buf[1]), len-2, ret);
    }
    return ret;
}
/*******************************************************
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    if (i >= 3)
    {
        ...
        {        
            ...
            {
                printk("Request processing, no esd recovery");
            }
            else
            {
                printk("IC working abnormally! Process esd recovery.");
                esd_buf[0] = 0x42;
                esd_buf[1] = 0x26;
                esd_buf[2] = 0x01;
                esd_buf[3] = 0x01;
...
/*		#ifdef GTP_CONFIG_OF
    ...
    #endif
        {
            //printk("IC working abnormally! Process reset guitar.");
            esd_buf[0] = 0x42;
            esd_buf[1] = 0x26;
            esd_buf[2] = 0x01;
            esd_buf[3] = 0x01;
...
static int __init  goodix_ts_init(void)
{
    ...
    GTP_DEBUG_FUNC();   
    printk("GTP driver installing...");
    printk("---<HERE WE IN THE GOODIX_TS_INIT>--\n");
    ...
    printk("----------->4\n");
    printk("THE VALUE OF i2c_add_driver=%d\n",ret);
    return ret; 
}
