...
#define RX8130_REG_Digital	0x30
...
static int rx8130_init_client(struct i2c_client *client, int *need_reset)
{
	...
	err = rx8130_write_reg(client, RX8130_REG_Digital, 0x00);
	if (err)
		goto out;
	...
	err = rx8130_write_reg(client, RX8130_REG_CTRL1, 0x10);
	if (err)
		goto out;
out:
	return err;
...
static int rx8130_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (need_reset) {
		...
		rtc_time_to_tm(1514793600, &tm);		// set to 2018/1/1
